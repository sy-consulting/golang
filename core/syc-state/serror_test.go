...
// ErrorCodeMatchCheck checks if error code from returned error matches expected error code returned
func ErrorCodeMatchCheck(t *testing.T, wantErr bool, expectedErrorCode interface{}, actualReturnedErr error) {
	if wantErr {
		if e, ok := actualReturnedErr.(SoteError); ok {
			if e.ErrCode != expectedErrorCode {
				t.Errorf("ValidToken() error-code = %v, expected-error-code = %v", e.ErrCode, expectedErrorCode)
			}
		} else {
			t.Errorf("error returned was not successfully cast: %v", actualReturnedErr)
		}
	}
}
